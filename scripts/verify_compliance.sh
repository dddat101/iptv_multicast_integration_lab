...
main() {
    ...
    if [[ $# -gt 0 && -f "$1" ]]; then
        pcap_file="$1"
        group="${2:-}"
    elif [[ $# -gt 0 && "$1" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        group="$1"
        pcap_file="${2:-}"
    else
        pcap_file="${1:-}"
    ...
    if [[ -n "${pcap_file}" && -f "${pcap_file}" ]] && check_command "${TSHARK_BIN:-tshark}"; then
        ...
        # TC_WIRE_02: IGMPv2 Membership Reports (Join signaling)
        local join_count
        join_count="$((tshark -r "${pcap_file}" -Y "igmp.type == 0x16 && igmp.maddr == ${group}" 2>/dev/null || true) | wc -l)"
        if (( join_count > 0 )); then
            check_test "TC_WIRE_02" "IGMPv2 Join Signaling" "PASS" "Detected ${join_count} Membership Report(s) for ${group}."
        else
            # Soft check for any IGMP join in case of multi-group test
            local any_join
        ...
        local data_count
        data_count="$((tshark -r "${pcap_file}" -Y "ip.dst == ${group} && udp.dstport == ${port}" 2>/dev/null || true) | wc -l)"
        ...
        if (( join_count > 0 && data_count > 0 )); then
            local t_join t_data
            t_join="$((tshark -r "${pcap_file}" -Y "igmp.type == 0x16 && igmp.maddr == ${group}" -T fields -e frame.time_epoch 2>/dev/null || true) | head -n1)"
            t_data="$((tshark -r "${pcap_file}" -Y "ip.dst == ${group} && udp.dstport == ${port}" -T fields -e frame.time_epoch 2>/dev/null || true) | awk -v j="${t_join}" 'j == "" || $1 >= j {print $1; exit}')"
        ...
        # TC_WIRE_05: IGMPv2 Leave Signaling (Fast Leave)
        local leave_count
        leave_count="$((tshark -r "${pcap_file}" -Y "igmp.type == 0x17 && igmp.maddr == ${group}" 2>/dev/null || true) | wc -l)"
        ...
        # TC_WIRE_06: IPv4 ToS/DSCP and Don't Fragment (DF) flags
